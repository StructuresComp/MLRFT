...
void elasticRod::setTwistBoundaryCondition(double desiredTwist, int k) // added for twist actuation
{
	undeformedTwist(k) = desiredTwist;
}
...
void elasticRod::setMass()
{
	...
	for (int i = 0; i < nv; i++)
	{
		// MASS ARRAY FOR THE DISTANCE BETWEEN FLAGELLA AND THE HEADNODE CORRECTED.
...
double elasticRod::getOmega(int k)
{
