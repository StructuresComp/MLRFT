...
void elasticTwistingForce::computeFt()
{
    ...
    {
        theta_e(i) = rod->x(4 * i + 3);
    }

    for (int i = 0; i < rod->ne - 1; i++)
    {
        theta_f(i) = rod->x(4 * i + 7);
    }
    ...
    for (int i = 1; i < rod->ne; i++)
    {
        ...
        {
            GJ2 = GJ * pow(10, 3);
            value = GJ2 / rod->voronoiLen(i - 1) * (deltam(i - 1) + rod->refTwist(i) - rod->undeformedTwist(i));
        }
        else
        {
            value = GJ / rod->voronoiLen(i - 1) * (deltam(i - 1) + rod->refTwist(i) - rod->undeformedTwist(i));
        }
        ci = 4 * i - 4;
...
void elasticTwistingForce::computeJt()
{
    for (int i = 1; i < rod->ne; i++)
    {
        ...
        milen = -1 / rod->voronoiLen(i - 1);

        if (i == 1 | i == 2)
        {
            GJ2 = GJ * pow(10, 3);
            Jtt = GJ2 * milen * ((deltam(i - 1) + rod->refTwist(i) - rod->undeformedTwist(i)) * DDtwist + gradTwistLocal * gradTwistLocal.adjoint());
        }
        else
        {
            Jtt = GJ * milen * ((deltam(i - 1) + rod->refTwist(i) - rod->undeformedTwist(i)) * DDtwist + gradTwistLocal * gradTwistLocal.adjoint());
        }
        for (int j = 0; j < 11; j++)
        {
            for (int k = 0; k < 11; k++)
